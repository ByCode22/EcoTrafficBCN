...
def on_mqtt_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        print(f"[MQTT] Subscrit a {TOPIC_COTXE}")
    else:
        print(f"[MQTT] ❌ Error connexió: codi {rc}")

def on_mqtt_disconnect(client, userdata, rc):
    print(f"[MQTT] ⚠️ Desconnectat: codi {rc}")
...
try:
    ...
    print(f"[MQTT] Connectant a {BROKER_IP}:{BROKER_PORT}...")
except Exception as e:
...
def set_semaforo(estado, force=False):
    ...
    if estado == "rojo":
        Leds.set_led1_color(True, False, False)
        Leds.set_led2_color(False, False, False)
    elif estado == "amarillo":
        Leds.set_led1_color(False, False, False)
        Leds.set_led2_color(True, True, False)
    elif estado == "verde":
        Leds.set_led1_color(False, False, False)
